...
void SmallBall::drawPickup(SDL_Renderer * renderer){
  SDL_RenderCopy(renderer,putexture,NULL,&pos);
}

void SmallBall::puEffector(IPBall * ball, IPPaddle * lpaddle, IPPaddle * rpaddle){
  bool addpos = true;
...
void SmallBall::deactivate(IPBall * ball, IPPaddle * lpaddle, IPPaddle * rpaddle){
  if((ball->pos.h != orig_h) && (ball->pos.w != orig_w)){
      ball->pos.h = orig_h;
      ball->pos.w = orig_w;
  }
